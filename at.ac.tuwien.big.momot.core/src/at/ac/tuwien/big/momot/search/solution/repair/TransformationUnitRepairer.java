...
public class TransformationUnitRepairer extends AbstractTransformationSolutionRepairer {
   ...
   public TransformationSolution repair(final TransformationSolution solution) {
      ...
      while(solution.getNumberOfVariables() > firstNotExecuted && solution.getVariable(firstNotExecuted).isExecuted()) {
         firstNotExecuted++;
      }

      if(firstNotExecuted == solution.getNumberOfVariables()) {
         return solution;
      }
